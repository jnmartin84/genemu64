...
extern size_t KosinskiDecompress(const unsigned long in_file_buffer, const unsigned long genbuf, size_t *out_file_size);
extern unsigned short CalcAngle(short x, short y);
void CalcSine(unsigned char angle, signed short *sin, signed short *cos);
extern unsigned char m68k_ram[65536];
extern unsigned char z80_ram[0x2000];	/* 8KB */
extern unsigned long in_file_pointer;
//extern uint16_t *m68k_ram16;
void KosDec()
{
	unsigned long src = REG_A[0];
	unsigned long dst = REG_A[1];
	size_t size;
	KosinskiDecompress(src, dst,&size);
	REG_A[0] = in_file_pointer;
	REG_A[1] = REG_A[1] + size;
}

static long signext(short b) {
	//if(b > 0x7f) return (0xFFFFFF00 | b);
	return (long)b;
//	else return (b & 0x000000FF);
}
extern const unsigned short sine_table[];
extern unsigned char *cromp;
//volatile extern int start_printing;
#if 0
static unsigned long signext(unsigned char b) {
	if(b > 0x7f) return (0xFFFFFF00 | b);
	else return (b & 0x000000FF);
}
...
{
loc_1523:
	REG_D[0] = /*signext*/(m68k_read_memory_8(REG_A[0]));
	REG_A[0] = REG_A[0] + 1;
	int cmpib;
loc_1530:
	cmpib = REG_D[0] == 0x80;
...
loc_1538:
	REG_D[7] = signext(REG_D[0]);
	// loc_153a
loc_153a:
    REG_D[0] = /*signext*/(m68k_read_memory_8(REG_A[0]));
    REG_A[0] = REG_A[0] + 1;
	
	if (REG_D[0]&0x000000FF > 0x7f) goto loc_1530;
	
	REG_D[1] = REG_D[0] & 0x000000FF;
	REG_D[7] = REG_D[7] & 0x0000000F;
	REG_D[1] = REG_D[1] & 0x00000070;
	REG_D[7] = REG_D[1] | REG_D[7];
	REG_D[0] = REG_D[0] & 0x0000000f;
	REG_D[1] = REG_D[0] & 0x000000ff;
	REG_D[1] = REG_D[1] << 8;
	REG_D[7] = REG_D[1] | REG_D[7];
	//REG_D[1] = 8;
	REG_D[1] = /*REG_D[1]*/ 8 - REG_D[0];
	
	if(REG_D[1] != 0) {
		//loc_1568
      REG_D[0] = m68k_read_memory_8(REG_A[0]);
      REG_A[0] = REG_A[0] + 1;
	  REG_D[0] = REG_D[0] << REG_D[1];
      REG_D[0] = REG_D[0]<<1; // + REG_D[0];
	  //REG_D[5] = 1;
	  REG_D[5] = /*REG_D[5]*/1 << REG_D[1];
	  REG_D[5] = REG_D[5] - 1;
loc_1574:
	  m68k_write_memory_16(REG_A[1] + m68k_read_memory_16(REG_D[0]), REG_D[7]);
	  REG_D[0] = REG_D[0] + 2;
	  REG_D[5] = REG_D[5] - 1;
	  if((signed)REG_D[5] >= 0) goto loc_1574;
	  else goto loc_153a;
	}

else {
      REG_D[0] = m68k_read_memory_16(REG_A[0]);
      REG_A[0] = REG_A[0] + 2;
	  REG_D[0] = REG_D[0]<<1; //+REG_D[0];
	  m68k_write_memory_16(REG_A[1] + REG_D[0], REG_D[7]);
	  goto loc_153a;
	}
  ...
  uint32 param_1 = REG_A[0];
  uint32 param_2 = REG_A[1];
  ...
  uVar3 = (uint16)m68k_read_memory_16(param_1);
  //*param_1;
  ...
  while (pbVar7 = pbVar6, uVar5 = uVar3, (signed char)uVar3 != -1) {
    while( 1 ) {
...
REG_A[0]++;    
	bVar1 = m68k_read_memory_8(pbVar7); //*pbVar7;
      ...
      if (uVar3 == 0) {
		...
		//*(ushort *)((int)param_2 + (int)(short)((ushort)m68k_read_memory_16(pbVar6) * 2)) = uVar5;
		m68k_write_memory_16((uint32)param_2 + (uint32)((ushort)m68k_read_memory_16(pbVar6) * 2), uVar5);
	  }
      else {
        pbVar7 = pbVar7 + 2;
        sVar2 = ((ushort)m68k_read_memory_16(pbVar6)) << ((uint)uVar3 & 0x3f) * 2;
        sVar4 = (1 << ((uint)uVar3 & 0x3f)) + -1;
        do {
//          *(ushort *)((int)param_2 + (int)sVar2) = uVar5;
        
m68k_write_memory_16((uint32)param_2 + (uint32)sVar2, uVar5);
...
extern int fine_grained_cycles;
int kos_cycles_used = 0;
int kos_start = 0;
int kos_end = 0;
...
//		m68ki_set_address_error_trap(); /* auto-disable (see m68kcpu.h) */
		...
		do
		{
			if(REG_PC == 0x189c) {
					kos_end = 0;
					kos_start = fine_grained_cycles;
			}
			else if(REG_PC == 0x193a) {
					kos_end = fine_grained_cycles;
					kos_cycles_used = kos_end - kos_start;
					kos_start = 0;
					kos_end = 0;
			}
...
// sonic 1
			if(REG_PC == 0x1440) {
				...
				special = 1;
				unsigned char* src = (unsigned char *)((unsigned long)(cromp + (REG_A[0])) | (unsigned long)0xA0000000);
				NemDec(src);
				USE_CYCLES(1500);//200);
					goto no_fake_continue;
			}
#endif
#if 0
// sonic 1
			else if(REG_PC == 0x1452) {
				special = 1;
				unsigned char* src = (unsigned char *)((unsigned long)(cromp + (REG_A[0])) | (unsigned long)0xA0000000);//cromp + (REG_A[0]);
				unsigned char* dst = (unsigned char *)(((unsigned long)m68k_ram + (REG_A[1] & 0xffff))| (unsigned long)0xA0000000);
				NemDecToRAM(src,dst);
				USE_CYCLES(1500);//200);
					goto no_fake_continue;
			}
...
#if 0
			else if(REG_PC == 0x29da) {
					special = 1;
//CalcSine(REG_D[0], &REG_D[0], &REG_D[1]);
unsigned char d0 = (unsigned char)(REG_D[0]&0xff);
//unsigned short *st = (unsigned short*)( (unsigned long)sine_table | 0xA0000000 );
//d0 = d0 * 2;

//d0 = d0 + 0x80;

REG_D[0] = (short)*(unsigned short*)(((unsigned long)sine_table | 0xA0000000) + (d0<<1));//signext( (short*)(&m68k_ram[0x2942 + d0]) );

//d0 = d0 - 0x80;
REG_D[1] = (short)*(unsigned short*)(((unsigned long)sine_table | 0xA0000000) + ((d0+0x40)<<1));
//(short)sine_table[d0+0x40];//signext( (short*)(&m68k_ram[0x2942 + d0]) );

					USE_CYCLES(100);
					goto no_fake_continue;
}
else if(REG_PC == 0x2ca8) {
					special = 1;
	REG_D[0] = CalcAngle(REG_D[1], REG_D[2]);
					USE_CYCLES(100);
					goto no_fake_continue;
}
#endif
...
//			m68ki_exception_if_trace(); /* auto-disable (see m68kcpu.h) */
				...
				// this just gets us to an RTS instruction
no_fake_continue:
//				printf("fake continue\n");
#if 1
// sonic 2
				//start_printing = 1;
				REG_PC = 0x19d8;
