...
void NemDec(const unsigned char *source);
void NemDecToRAM(const unsigned char *source, unsigned char *destination);
...
extern void CalcSine(unsigned char angle, signed short *sin, signed short *cos);
...
extern int in_vdp_interrupt;
#define HLE 0

#if HLE
int HLE_running = 0;
int HLE_cycles_remaining = 0; 
static unsigned long HLE_PC = 0xFFFFFFFF;
static unsigned long HLE_RTS = 0xFFFFFFFF; // pc of rts belonging to current subroutine
#define SONIC2
#ifdef SONIC1
#define PC_NemDec 0x00001440 
#define PC_NemDecToRAM 0x00001452
#define PC_KosDec 0x0000189C
#define PC_CalcSine 0x000029DA
#define PC_CalcAngle 0x00002CA8
#endif
#ifdef SONIC2 
#define PC_KosDec 0x0000193A
#endif
#endif
		...
		if(!in_vdp_interrupt) {
			...
			else
			{
				...
				switch(HLE_PC)
				{
					...
					{
						HLE_RTS = 0x00001488;
						HLE_cycles_remaining = 10;//800000;

						unsigned char* src = (unsigned char *)(rom + (REG_A[0]));
						NemDec(src);
					...
					case PC_NemDecToRAM:
					{
						...
						unsigned char* src = (unsigned char *)(rom + (REG_A[0]));
						unsigned char* dst = (unsigned char *)(m68k_ram + (REG_A[1] & 0xffff));
						NemDecToRAM(src,dst);
					...
					case PC_KosDec:
					{
						...
						HLE_RTS = 0x0000193A;
#endif
						HLE_cycles_remaining = 10;//1000000;
					...
					{
						HLE_RTS = 0x000029F0;
						HLE_cycles_remaining = 10;//400;
						
						unsigned char d0 = (unsigned char)(REG_D[0]&0xff);
						REG_D[0] = (short)*(unsigned short*)((unsigned long)sine_table + (d0<<1));
						REG_D[1] = (short)*(unsigned short*)((unsigned long)sine_table + ((d0+0x40)<<1));						
//						CalcSine(REG_D[0], (short*)&REG_D[0], (short*)&REG_D[1]);
				...
				if (HLE_RTS != 0xFFFFFFFF)
				{
					HLE_running = 1;
					HLE_PC = REG_PC;

					USE_CYCLES(HLE_cycles_remaining > 488 ? 488 : HLE_cycles_remaining);
					
					if (HLE_cycles_remaining > 488)
					{
						return 0;
					}
					else
					{
						HLE_cycles_remaining = 0;
						HLE_running = 0;
						REG_PC = HLE_RTS;
					}
				}
		...
		do
		{
			...
			// KosDec 500K - 2M cycles
/*			if(REG_PC == 0x189c) {
			else if(REG_PC == 0x193a) {
			}*/
			
			// CalcSine 400 - 500 cycles
/*			if(REG_PC == 0x29DA) {
			else if(REG_PC == 0x29f0) {
			}*/
			
			// CalcAngle 150 - 300 cycles
/*			if(REG_PC == 0x2CA8) {
			}
			else if(REG_PC == 0x2D02 || REG_PC == 0x2D0E) {
			}*/
