...
{
	if(1 && 	!((((int)dest | (int)src | n) & 7)) && n>15) {

//	!((uint32_t)dest&7) && !((uint32_t)src&7) && !((uint32_t)n&7) && (n>16)) {
#if 1
	int rsp_block_size;

	if (n > 511) rsp_block_size = 512;
...
//	IO_WRITE(SP_DMA_SEM_REG, 0); // clear semaphore
	...
	D_memcpy(dest+(blocks*rsp_block_size), src+(blocks*rsp_block_size), leftover);
...
uint16_t *ocr_vram;
static uint8_t __attribute__((aligned(8))) tile_data[sizeof(sprite_t) + (64*32)];
...
uint8_t palnum,palnum2,palnum3,palnum4;
	...
	//	IO_WRITE(0x04000000 + 768, (uintptr_t)__safe_buffer[(_dc)-1] + (y*640) + (x*2));
	...
	//		draw=draw2=draw3=draw4=0;
			//uint32_t ent_32= *(uint32_t*)(vdp_bgb[row + ((col_off + ((/*pix_off +*/ x) >> 3)) % vdp_sc_width)];
			//uint32_t ent2_32= *(uint32_t*)(vdp_bga[row2 + ((col_off2 + ((/*pix_off2 +*/ x) >> 3)) % vdp_sc_width)]);
			uint16_t name_ent = vdp_bgb[row + ((col_off + ((pix_off + x) >> 3)) % vdp_sc_width)];//(ent_32 >> 16)&0xffff;//vdp_bgb[row + ((col_off + ((pix_off + x) >> 3)) % vdp_sc_width)];
//			uint16_t name_ent1 = vdp_bgb[row + ((col_off + ((pix_off + (x+8)) >> 3)) % vdp_sc_width)];//(ent_32 & 0xffff);//vdp_bgb[row + ((col_off + ((pix_off + x) >> 3)) % vdp_sc_width)];
			uint16_t name_ent2 = vdp_bga[row2 + ((col_off2 + ((pix_off2 + x) >> 3)) % vdp_sc_width)];
//			uint16_t name_ent21 = vdp_bga[row2 + ((col_off2 + ((pix_off2 + (x+8)) >> 3)) % vdp_sc_width)];
...
/*			if (((name_ent1 >> 15) == 1) && ((name_ent21 >> 15)==0)) {
				uint16_t tmp = name_ent21;
				name_ent21 = name_ent1;
				name_ent1 = tmp;
			}*/
//if((name_ent2 >> 15) == 0)			
//			{
//				draw = 1;
				...
				/*uint32_t tilenum3;// = ((name_ent & 0x7ff) << 5);	
				uint32_t tilenum4;// = ((name_ent2 & 0x7ff) << 5);	
				tilenum3 = ((name_ent1 & 0x7ff) << 5);	
				tilenum4 = ((name_ent21 & 0x7ff) << 5);						
				*/
...
*/
if(/*last_vf2 != rsp_vf1 || last_hf2 != rsp_vf1 || last_vf != rsp_vf0 || last_hf != rsp_hf0 ||*/ tilenum2 != last_tilenum2 || tilenum != last_tilenum)
				{			
	...
	/*				if((name_ent2 >> 15) == 0 && (name_ent >> 15) == 1)
		...
		*/			{
					...
					rsp_combine_tiles(pix_off, pix_off2);
					rdp_load_texture(tile0_sprite, 0);
/*last_vf = rsp_vf0;
last_hf = rsp_hf0;
last_vf2 = rsp_vf1;
last_hf2 = rsp_hf1;*/
...
#if 0
				if(tilenum3 != last_tilenum3 || tilenum4 != last_tilenum4)
				{			
					rsp_sprite_data = (uintptr_t)tile1_sprite->data | 0xA0000000;
					rsp_addr0 = (uint32_t)vdp_vram + tilenum3;
					rsp_addr1 = (uint32_t)vdp_vram + tilenum4;
	...
	/*				if((name_ent2 >> 15) == 0 && (name_ent >> 15) == 1)
		...
		*/			{
						rsp_vf0 = (uint32_t)((name_ent1 >> 12) & 0x1);
						rsp_hf0 = (uint32_t)((name_ent1 >> 11) & 0x1);
						rsp_pn0 = (uint32_t)((name_ent1 >> 9) & 0x30);

						rsp_vf1 = (uint32_t)((name_ent21 >> 12) & 0x1);
						rsp_hf1 = (uint32_t)((name_ent21 >> 11) & 0x1);
						rsp_pn1 = (uint32_t)((name_ent21 >> 9) & 0x30);
					}

					rsp_combine_tiles();

					last_tilenum3 = tilenum3;
					last_tilenum4 = tilenum4;
				}
#endif
				//}
iii++;
...
//			if(draw)
#if 0
				//uint32_t* buf32 = (uint32_t*)((uintptr_t)__safe_buffer[(_dc)-1] + (y*640) + (x*2));
for(int yy=0;yy<8;yy++) {
for(int xx=0;xx<8;xx+=2) {	
/*D_memcpy( (uintptr_t)__safe_buffer[(_dc)-1] + ((((y+0)*320) + x)<<1), ((uintptr_t)&tile0_sprite->data | 0xA0000000) + (0*16), 16);
D_memcpy( (uintptr_t)__safe_buffer[(_dc)-1] + ((((y+1)*320) + x)<<1), ((uintptr_t)&tile0_sprite->data | 0xA0000000) + (1*16), 16);
D_memcpy( (uintptr_t)__safe_buffer[(_dc)-1] + ((((y+2)*320) + x)<<1), ((uintptr_t)&tile0_sprite->data | 0xA0000000) + (2*16), 16);
D_memcpy( (uintptr_t)__safe_buffer[(_dc)-1] + ((((y+3)*320) + x)<<1), ((uintptr_t)&tile0_sprite->data | 0xA0000000) + (3*16), 16);
D_memcpy( (uintptr_t)__safe_buffer[(_dc)-1] + ((((y+4)*320) + x)<<1), ((uintptr_t)&tile0_sprite->data | 0xA0000000) + (4*16), 16);
D_memcpy( (uintptr_t)__safe_buffer[(_dc)-1] + ((((y+5)*320) + x)<<1), ((uintptr_t)&tile0_sprite->data | 0xA0000000) + (5*16), 16);
D_memcpy( (uintptr_t)__safe_buffer[(_dc)-1] + ((((y+6)*320) + x)<<1), ((uintptr_t)&tile0_sprite->data | 0xA0000000) + (6*16), 16);
D_memcpy( (uintptr_t)__safe_buffer[(_dc)-1] + ((((y+7)*320) + x)<<1), ((uintptr_t)&tile0_sprite->data | 0xA0000000) + (7*16), 16);
*/
//	buf32[((yy*320) + xx)>>1] = 
*(uint32_t*)((uintptr_t)__safe_buffer[(_dc)-1] + ((yy+y)*640) + ((xx+x)*2)) = 
			*(uint32_t*)(((uintptr_t)&tile0_sprite->data | 0xA0000000) + ((yy*16) + (xx*2)));
