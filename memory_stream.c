...
static cc_bool_fast ResizeIfNeeded(MemoryStream *memory_stream, size_t minimum_needed_size)
{
	if (minimum_needed_size > memory_stream->size)
	{
		...
		if(minimum_needed_size < 65536) new_size = 65536;
		else 	{	while (new_size < minimum_needed_size) {
		new_size <<= 1; } }
...
//		buffer = (unsigned char*)realloc(memory_stream->buffer, new_size);
	...
	//	D_memset(memory_stream->buffer + memory_stream->size, 0, new_size - memory_stream->size);
...
#endif
void MemoryStream_Create(MemoryStream *memory_stream, unsigned long genbuf, cc_bool_fast free_buffer_when_destroyed)
{
//	if(genbuf >= 0xFF0000 && genbuf <= 0xFFFFFF) {
		
	memory_stream->buffer = genbuf;//(unsigned char *)(m68k_ram + (genbuf & 0xFFFF));
//	}
//	else 
...
//		return CC_FALSE;
//if(memory_stream->buffer >= 0xFF0000 && memory_stream->buffer <= 0xFFFFFF) {
*(unsigned char *)(m68k_ram + (memory_stream->buffer & 0xFFFF) + memory_stream->position++) = byte;
//}		//memory_stream->buffer[memory_stream->position++] = byte;
//else {
//	m68k_write_memory_8(memory_stream->buffer + memory_stream->position++, byte);
//}
	return CC_TRUE;
}
...
//	D_memcpy(&memory_stream->buffer[memory_stream->position], data, size * count);
...
size_t MemoryStream_Read(MemoryStream *memory_stream, void *output, size_t size, size_t count)
{
	...
	//D_memcpy(output, &memory_stream->buffer[memory_stream->position], size * count);
	memory_stream->position += size * count;

	return count;
}

unsigned char MemoryStream_GetBuffer(MemoryStream *memory_stream, size_t offset)
{
//if(memory_stream->buffer >= 0xFF0000 && memory_stream->buffer <= 0xFFFFFF) {
return *(unsigned char *)(m68k_ram + (memory_stream->buffer & 0xFFFF) + offset);
//}		//memory_stream->buffer[memory_stream->position++] = byte;
//else {
//	return m68k_read_memory_8(memory_stream->buffer + offset);
